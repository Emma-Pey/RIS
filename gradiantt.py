...
if N != N_SIDE ** 2:
    raise ValueError("N doit etre egal a N_SIDE ** 2 pour former la grille RIS.")

if M > len(user_pos):
    raise ValueError(f"M={M} depasse le nombre de positions utilisateurs disponibles ({len(user_pos)}).")
...
def optimize():
    ...
    print(f"  Duree       : {time_hist[-1]:.3f} s")
    print(f"{'='*60}\n")
    return theta, SR_hist, grad_hist, time_hist, SR0, SR_final
...
# ── Execution ─────────────────────────────────────────────────────────────────
theta_opt, SR_hist, grad_hist, time_hist, SR0, SR_final = optimize()
...
ax = axes[0, 1]
ax.plot(time_hist, SR_hist, 'seagreen', lw=2)
ax.axhline(SR0,      color='gray',   ls='--', label=f'Initial : {SR0:.2f} bps/Hz')
ax.axhline(SR_final, color='tomato', ls='--', label=f'Final   : {SR_final:.2f} bps/Hz')
ax.set_xlabel('Duree (s)'); ax.set_ylabel('Debit somme (bps/Hz)')
ax.set_title('Convergence — debit somme vs duree')
ax.legend(); ax.grid(alpha=0.3)
...
ax = axes[1, 2]
ax.plot(time_hist, grad_hist, 'purple', lw=2)
ax.set_xlabel('Duree (s)'); ax.set_ylabel('||grad J||')
ax.set_title('Norme du gradient vs duree')
ax.grid(alpha=0.3)
